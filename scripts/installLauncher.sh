...
set -e
paperworkdir="$basedir/Purpur/Paper/work"
mcver=$(cat "$paperworkdir/BuildData/info.json" | grep minecraftVersion | cut -d '"' -f 4)
serverjar="$basedir/RTGaming-Server/target/rtgaming-$mcver.jar"
...
cp "$paperworkdir/Paperclip/assembly/target/paperclip-${mcver}.jar" "$basedir/rtgaming-${mcver}-paperclip.jar"

echo ""
echo "  $(bashcolor 1 32)Success$(bashcolorend) - Saved launcher jar to target/rtgaming-${mcver}-launcher.jar"
